...
struct
  ...
  (*given a string and an outstring, prints string to outstream*)
  ...
  (*given a type returns corresponding JVM letter*)
  ...
  (*helper function to write types for JVM to handle fucntions*)
  fun getFunArgs(params : A.paramdecl list) : string =
    case params of
      [] => ""
    | (t,id)::ps => getFunType(t) ^ getFunArgs(ps)

  (*given a type returns either i or d to attach to JVM instruction*)
  ...
  (*given nextLabel, returns a string and increments nextLabel*)
  fun genLabel(n : nextLabel) : string * nextLabel =
    ("LBL" ^ Int.toString(n),n + 1)

  (*handles compiling expressions that involve comparisons*)
  fun compareHelper (t: A.typ, e0: A.exp, e1: A.exp, ord: order, 
                    env: compileenv, outs: T.outstream) : compileenv =
    case getTyp(e0) of
 ...
 (*given an expression, outstream, and an environment, 
  compiles expressions and returns updated environment*)
  and compileExp(exp : A.exp, outs: T.outstream, env : compileenv) 
                 : compileenv =
    case exp of
      A.EInt(x) => 
        let val a = writeString(("ldc " ^ Int.toString(x)), outs)
              in
                env
              end
    | A.EDouble(x) => 
        let val a = writeString("ldc2_w " ^ Real.toString(x), outs ) 
              in
                env
              end
    | A.EString(s) => raise Unimplemented
    | A.ETrue => 
        let val a = writeString("ldc 1", outs) 
              in
                env
              end
    | A.EFalse => 
        let val a = writeString("ldc 0", outs) 
              in
                env
              end
    | A.EId(id,t) => 
        let val (locs,nextLoc,nextLabel,fncs) = env
            ...
            val a = writeString(getPrefix(t) ^ "load " ^ Int.toString(idloc), 
                    outs) 
              in
                env
              end
    | A.ECall((id,exps),t) => 
        (case id of
          "readInt" => 
              let val (locs,nextLoc,nextLabel,fncs) = 
                            compileExpList(exps,outs,env)
    ...
    | A.EPreIncr(id,t) => 
              let val (locs,nextLoc,nextLabel,fncs) = env
                  val idloc = Env.lookup locs id
                  val a = writeString(getPrefix(t) ^ "load " ^ 
                          Int.toString(idloc),outs)
                  val b = writeString("ldc 1",outs)
    ...
    | A.ENot(e,t) => 
              let val (locs,nextLoc,nextLabel,fncs) = compileExp(e,outs,env)
    ...
    | A.EMul((e0,e1),t) => 
                let val env1 = compileExp(e0,outs,env)
                    val env2 = compileExp(e1,outs,env1)
                    val a = writeString(getPrefix(t) ^ "mul",outs) 
                      in
                        env2
                      end
    | A.EDiv((e0,e1),t) => 
                let val env1 = compileExp(e0,outs,env)
                    val env2 = compileExp(e1,outs,env1)
                    val a = writeString(getPrefix(t) ^ "div",outs) 
                      in
                        env2
                      end
    | A.EMod((e0,e1),t) => 
                let val env1 = compileExp(e0,outs,env)
                    val env2 = compileExp(e1,outs,env1)
                    val a = writeString(getPrefix(t) ^ "rem",outs) 
                      in
                        env2
                      end
    | A.EAdd((e0,e1),t) => 
                let val env1 = compileExp(e0,outs,env)
                    val env2 = compileExp(e1,outs,env1)
                    val a = writeString(getPrefix(t) ^ "add",outs) 
                      in
                        env2
                      end
    | A.ESub((e0,e1),t) => 
                let val env1 = compileExp(e0,outs,env)
                    val env2 = compileExp(e1,outs,env1)
                    val a = writeString(getPrefix(t) ^ "sub",outs) 
                      in
                        env2
                      end
    ...
    | A.ECond((e0,e1,e2),t) => 
                let val (locs,nextLoc,nextLabel,fncs) = compileExp(e0,outs,env)
                    val (falselbl,label0) = genLabel(nextLabel)
                    val a = writeString("ifeq " ^ falselbl,outs)
                    val (locs',nextLoc',nextLabel',fncs') = 
                          compileExp(e1,outs,(locs,nextLoc,label0,fncs))
                    val (truelbl,label1) = genLabel(nextLabel')
                    val b = writeString("goto " ^ truelbl,outs)
                    val c = writeString(falselbl ^ ":",outs)
                    val env'' = 
                          compileExp(e2,outs,(locs',nextLoc',label1,fncs'))
                    val d = writeString(truelbl ^ ":",outs)
                      in
                        env''
                      end
  (*comiles a list of expressions*)
  and compileExpList(exps : A.exp list, outs : T.outstream, env : compileenv)
                    : compileenv =
    case exps of
      [] => env
  ...
  (*given a type, increments nextloc appropriate amount*)
  ...
  (*compiles statement*)
  fun compileStm (stm : A.stm, outs : T.outstream, env : compileenv, 
                  returnt : A.typ option) : compileenv =
    ...
    | A.SDecl(t,ids) => 
        (case ids of
          [] => env
          | id::xs => 
            let val newloc = Env.extend locs id nextLoc
                  in
    ...
    | A.SFor((t,id,e0),e1,e2,s) => 
            let val (locs',nextLoc',nextLabel',fncs') = compileExp(e0,outs,env)
                ...
                val (locs'',nextLoc'',nextLabel'',fncs'') =
                      compileExp(e1,outs,
                      (newloc,incNextLoc(nextLoc',t),label0,fncs'))
    ...
    | A.SIfElse(e,s0,s1) => 
              let val (locs,nextLoc,nextLabel,fncs) = compileExp(e,outs,env)
                  ...
                  val a  = writeString("ifeq " ^ falselbl,outs)
                  val (locs',nextLoc',nextLabel',fncs') = 
                        compileStm(s0,outs,(locs,nextLoc,label0,fncs),returnt)
                  val (truelbl,label1) = genLabel(nextLabel')
                  val b = writeString("goto " ^ truelbl,outs)
                  val c = writeString(falselbl ^ ":",outs)
                  val env' = compileStm(s1,outs,
                              (locs',nextLoc',label1,fncs'),returnt)
                  val d = writeString(truelbl ^ ":",outs)
                    in
                      env'
    ...
    | A.SReturn(e) => 
              (case returnt of
                  ...
                  | SOME(t) => let val env' = compileExp(e,outs,env)
                                   val () = writeString(getPrefix(t) ^ 
                                              "return",outs)
                                    in
                                      env'
                                    end)
    | A.SVReturn => 
              let val a = writeString("return",outs)
                    in
                      env
                    end
    | A.SBlock(slist) => 
  ...
  (*compiles a list of statements, returns updated environment*)
  and compileStmList(stms : A.stm list, outs : T.outstream, env : compileenv, 
                     returnt : A.typ option) : compileenv =
    case stms of
      [] => env
    | s::ss => 
        let val newenv = compileStm(s,outs,env,returnt)
              in
  ...
  (*compiles a definition and returns updated environment*)
  fun compileDef(d : A.def, outs : T.outstream, env : compileenv) 
                 : compileenv =
    case d of
        A.DFun(t,id,params,stms) =>
            let val newenv = updateEnv(env,params)
                ...
                val (_,_,nextLabel,fncs'') = 
                    (case id of
                      "main" => 
                        compileStmList(stms,outs,
                          (locs,nextLoc,nextLabel,fncs'),NONE)
                      | _ => compileStmList(stms,outs,
                                (locs,nextLoc,nextLabel,fncs'),SOME(t)))
  ...
  (*compiles a definition list and returns an updated environment*)
  fun compileDefList(defs : A.def list, outs : T.outstream, env : compileenv) 
                     : compileenv =
    case defs of
      [] => env
  ...
  *)
  fun compile(A.PDefs(deflist) : A.program, outs : T.outstream) : unit =
    let val () = writeString(".class public C",outs)
        ...
        val () = writeString("invokenonvirtual java/lang/Object/<init>()V"
                  ,outs)
        val () = writeString("return",outs)
        val () = writeString(".end method",outs)
        val env = compileDefList(deflist, outs,
                    ((Env.pushFrame Env.empty Frame.empty)
